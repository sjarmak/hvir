...
- **A crew refresh has a fast half and a slow half, and only the fast half may poll.**
  The session list is live and re-read every tick. The city's shape — rigs, city root,
  resolved config — changes when someone edits config, i.e. never on a 4-second timescale.
