...
`terminal-move`, `workbench-layout`, `terminal-split`, `app-settings`, `harness-profiles`, and
`attention-away-throttling` groups in separate Electron processes with fresh project and user-data roots, then
...
`harness-profiles`, `attention-away-throttling`, and `capacity`. The
